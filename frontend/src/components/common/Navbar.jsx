...
const ThemePicker = ({ activeTheme, switchTheme, accent }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setIsOpen(!isOpen)}
        className="w-8 h-8 flex items-center justify-center rounded-sm transition-all text-white/50 hover:text-white hover:bg-white/8">
        <Palette size={14} />
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-[calc(100%+8px)] right-0 w-48 bg-[#0e0e0e] border border-white/10 rounded-sm shadow-2xl shadow-black/60 overflow-hidden z-[300]"
          >
            ...
            <div className="py-1">
              {Object.entries(THEMES).map(([key, theme]) => (
                <button key={key} onClick={() => { switchTheme(key); setIsOpen(false); }}
                  ...
                  {activeTheme === key && <span className="ml-auto font-mono text-[8px] text-white/40">✓</span>}
                </button>
              ))}
            </div>
          </motion.div>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${
        ...
        <div className="max-w-screen-2xl mx-auto px-4 md:px-8 h-16 flex items-center gap-4">
          ...
          <Link to="/" className="flex-shrink-0 flex items-center gap-1 group mr-2">
            <span className="font-mono text-white font-bold tracking-[0.12em] text-sm uppercase group-hover:opacity-80 transition-opacity">Cinema</span>
            <span className="font-mono font-bold tracking-[0.12em] text-sm uppercase" style={{ color: accent }}>Trial</span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link key={link.name} to={link.path}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-sm font-mono text-[10px] tracking-[0.15em] uppercase transition-all duration-150 text-white/50 hover:text-white hover:bg-white/8"
                style={isActive(link.path) ? { backgroundColor: accent, color: accentText, fontWeight: "700" } : {}}>
                <link.icon size={11} />
                {link.name}
              </Link>
            ))}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="hidden md:block">
              <ThemePicker activeTheme={activeTheme} switchTheme={switchTheme} accent={accent} />
            </div>
            ...
            <div className="relative" ref={searchRef}>
              <div className="flex items-center">
                <AnimatePresence>
                  {isSearchOpen && (
                    ...
                    >
                      <input ref={searchInput} type="text" placeholder="Search films, shows..."
                        value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full bg-white/8 border border-white/15 px-3 py-1.5 font-mono text-[11px] tracking-wider text-white outline-none placeholder-white/25 rounded-sm" />
                    </motion.form>
                  )}
                </AnimatePresence>
                <button onClick={() => setIsSearchOpen(!isSearchOpen)}
                  className="w-8 h-8 flex items-center justify-center rounded-sm transition-all flex-shrink-0 text-white/50 hover:text-white hover:bg-white/8"
              ...
              <AnimatePresence>
                {isSearchOpen && (searchResults.length > 0 || searchLoading || searchQuery.trim()) && (
                  ...
                  >
                    {searchLoading && <div className="px-4 py-3 font-mono text-[10px] text-white/30 uppercase animate-pulse">Searching...</div>}
                    {!searchLoading && searchResults.length === 0 && searchQuery.trim() && (
                      <div className="px-4 py-3 font-mono text-[10px] text-white/30 uppercase">No results</div>
                    )}
                    {!searchLoading && searchResults.map((item) => (
                      <button key={item.id} onClick={() => handleResultClick(item)}
                        className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-white/5 transition-colors text-left border-b border-white/5 last:border-0 group">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-mono text-[11px] text-white truncate group-hover:text-[var(--color-accent)] transition-colors">{item.title || item.name}</p>
                    ...
                    {searchResults.length > 0 && (
                      <button onClick={handleSearchSubmit}
                        className="w-full px-4 py-2.5 border-t border-white/8 font-mono text-[10px] hover:bg-white/5 transition-colors uppercase text-left"
                        style={{ color: accent }}>
                        See all results →
                      </button>
                    )}
            ...
            {isAuth ? (
              <div className="relative hidden md:block" ref={dropdownRef}>
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  className="flex items-center gap-2 pl-2.5 pr-2 py-1.5 rounded-sm font-mono text-[10px] tracking-[0.12em] uppercase transition-all text-white/60 hover:text-white hover:bg-white/8"
                  ...
                  {isDropdownOpen && (
                    ...
                    >
                      <div className="px-4 py-3 border-b border-white/8">
                        ...
                        <p className="font-mono text-[11px] text-white mt-0.5 truncate">{user?.email}</p>
                      </div>
                      <div className="py-1">
              ...
              <div className="hidden md:flex items-center gap-2">
                ...
                <Link to="/register" className="px-4 py-1.5 font-mono text-[10px] tracking-[0.15em] uppercase font-bold hover:opacity-90 transition-opacity rounded-sm"
                  style={{ backgroundColor: accent, color: accentText }}>Register</Link>
              </div>
            )}
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setIsMenuOpen(false)}
                    className="w-7 h-7 flex items-center justify-center text-white/40 hover:text-white hover:bg-white/8 rounded-sm">
                    <X size={15} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto py-2">
                {navLinks.map((link) => (
                  <Link key={link.name} to={link.path} onClick={() => setIsMenuOpen(false)}
                    className="flex items-center gap-3 px-5 py-3.5 font-mono text-[11px] tracking-[0.2em] uppercase transition-colors text-white/60 hover:text-white hover:bg-white/5"
                    style={isActive(link.path) ? { color: accent, backgroundColor: `${accent}18` } : {}}>
