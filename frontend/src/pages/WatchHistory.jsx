              ...
              {history.map((item) => (
                ...
                >
                  <div className="w-20 h-28 flex-shrink-0 border border-border overflow-hidden">
                    <img
                      src={item.posterPath ? `${IMG_BASE}/w200${item.posterPath}` : "/no-poster.png"}
                      alt=""
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="flex items-center gap-2 font-mono text-[10px] text-muted uppercase tracking-widest">
                      <Clock size={12} />
                      <span>Watched {new Date(item.createdAt).toLocaleString()}</span>
                    </div>
                  </div>
                </motion.div>
              ))}
      ...
      >
        <div className="space-y-8">
          <p className="font-body text-muted">
            ARE YOU SURE YOU WANT TO WIPE YOUR WATCH HISTORY? THIS ACTION IS PERMANENT AND CANNOT BE UNDONE.
          </p>
          <div className="flex gap-4">
            <Button variant="danger" className="flex-grow" onClick={handleClearHistory}>
              YES, WIPE IT
            </Button>
